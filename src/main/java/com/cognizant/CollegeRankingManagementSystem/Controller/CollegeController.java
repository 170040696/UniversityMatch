...
public class CollegeController {
	...
	public ModelAndView addCollege(Model model) {
		College college = new College();

		
		model.addAttribute("college", college);
		 ModelAndView modelAndView = new ModelAndView("add_college");
		...
		System.out.println("In enter College details page");	
		return modelAndView;
	}
	...
	public ModelAndView saveCollege(@ModelAttribute("college") College college) {
		...
		System.out.println("In Save College details to database");
	...
	@PostMapping("UpdateTheCollege")
	public ModelAndView saveCollege1(@ModelAttribute("college") College college) {
	
		ScoreGenerator scoreGenerator = new ScoreGenerator();
		System.out.println("In addCollege2");
		// Calculating the scores using scoreGenerator. Functions are present in the service package.
		college.setOverallScore(scoreGenerator.generateOverallScore(college));
		college.setResearchScore(scoreGenerator.generateResearchScore(college));
		college.setEmploymentScore(scoreGenerator.generateEmploymentScore(college));
	...
	//@RequestMapping("adminFrontPage")
	@GetMapping("adminFrontPage/{email}")
	public ModelAndView toCollegeRegistration(Model model, @PathVariable String email) {
	//public ModelAndView toCollegeRegistration() {
		ModelAndView modelAndView = new ModelAndView();
		College c = collegeRepository.findByAdmissionEmail(email);
		...
		{
			modelAndView.setViewName("admin_front_page2");
			
		}
		modelAndView.addObject("collegeEmail", email);
	    modelAndView.addObject("collegeId",c.getCollegeId());
	    System.out.println("Collefe ID is "+ c.getCollegeId());
		modelAndView.addObject("apiKey", collegeAuthentication.getApiKey());
		modelAndView.addObject("authDomain", collegeAuthentication.getAuthDomain());
		modelAndView.addObject("projectId", collegeAuthentication.getProjectId());
		modelAndView.addObject("storageBucket", collegeAuthentication.getStorageBucket());
	...
	public ModelAndView deleteCollege(@RequestParam int collegeId) {
		...
		Optional<College>c=collegeRepository.findById(collegeId);
		
		
		System.out.println("Printing the College");
		ModelAndView modelAndView = new ModelAndView();
		if(c.isPresent()) {
			collegeRepository.deleteById(collegeId); // The JPA Repository has this specific function to delete the college using college ID.
		modelAndView.setViewName("admin_front_page2");
		}
		else {
			modelAndView.setViewName("No_Colg_ToDelete");	
		}
		modelAndView.addObject("apiKey", collegeAuthentication.getApiKey());
		modelAndView.addObject("authDomain", collegeAuthentication.getAuthDomain());
		modelAndView.addObject("projectId", collegeAuthentication.getProjectId());
